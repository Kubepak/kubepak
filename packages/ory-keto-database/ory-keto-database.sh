...
# @package-option base-packages="postgresql" [ ,${CONTEXT}, =~ ,ory-keto-local-database, ]

#-----------------------------------------------------------------------------
# Public Hooks

hook_initialize() {
    if [[ ,${CONTEXT}, =~ ,ory-keto-local-database, ]]; then
        ...
        package_cache_values_file_write ".packages.${PACKAGE_IPATH}.metadata.parameters" "sslmode=disable" true
    else
