...
hook_initialize() {
    package_cache_values_file_write ".packages.${PACKAGE_IPATH}.chartVersion" "${__CERT_MANAGER_CHART_VERSION}"
...
hook_install() {
    ...
    k8s_resource_wait "${K8S_PACKAGE_NAMESPACE}" "deployment" "${K8S_PACKAGE_NAME}"
    k8s_resource_wait "${K8S_PACKAGE_NAMESPACE}" "deployment" "${K8S_PACKAGE_NAME}-cainjector"
    k8s_resource_wait "${K8S_PACKAGE_NAMESPACE}" "deployment" "${K8S_PACKAGE_NAME}-webhook"
    k8s_resource_wait "${K8S_PACKAGE_NAMESPACE}" "validatingwebhookconfigurations" "${K8S_PACKAGE_NAME}-webhook"
    k8s_resource_wait "${K8S_PACKAGE_NAMESPACE}" "secret" "${K8S_PACKAGE_NAME}-webhook-ca"

    k8s_resource_wait_for "${K8S_PACKAGE_NAMESPACE}" "job.batch" "${K8S_PACKAGE_NAME}-startupapicheck" "condition=complete" "120s"
}
