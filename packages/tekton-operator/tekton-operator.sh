...
readonly __TEKTON_OPERATOR_CHART_VERSION="main"

#-----------------------------------------------------------------------------
# Public Hooks

hook_initialize() {
    package_cache_values_file_write ".packages.${PACKAGE_IPATH}.chartVersion" "${__TEKTON_OPERATOR_CHART_VERSION}"

    k8s_namespace_create "${K8S_PACKAGE_NAMESPACE}"

    registry_credentials_add_namespace "${K8S_PACKAGE_NAMESPACE}"

    curl -sSfL "https://raw.githubusercontent.com/tektoncd/operator/${__TEKTON_OPERATOR_CHART_VERSION}/charts/tekton-operator/templates/kubernetes-crds.yaml" |
        sed -e '1d' -e '$ d' |
        kubectl apply -f -
}
